...
class Prog::Test::Kubernetes < Prog::Test::Base
  ...
  def ensure_hosts_entry(sshable, api_hostname)
    host_line = "#{sshable.host} #{api_hostname}"
    output = sshable.cmd("cat /etc/hosts")
    unless output.include?(host_line)
      sshable.cmd("echo '#{host_line}' | sudo tee -a /etc/hosts > /dev/null")
    end
  end
  ...
  label def wait_kubernetes_resource
    if kubernetes_cluster.strand.label == "wait"
      ...
      [kubernetes_cluster.cp_vms, kubernetes_cluster.worker_vms].each do |vm_group|
        if vm_group.length == 1
          ensure_hosts_entry(vm_group.first.sshable, api_hostname)
        end
      end
      nap 10
    end
